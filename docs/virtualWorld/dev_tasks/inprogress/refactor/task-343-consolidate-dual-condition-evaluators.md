# task-343 — Consolidate the two divergent condition evaluators into one

**Status**: Todo — filed 2026-08-27 from code review. Priority: do BEFORE the
16-condition sweep (todo/conditions/*) — otherwise every new condition must be
...
1. `engine/triggers/condition_tree.py::_evaluate_conditions` — ~25 leaf types,
   string-keyed if/elif chain. Used by the main trigger system.
2. `engine/triggers/evaluation.py:91` `_evaluate_trigger_condition`
   (EvaluationMixin) — duplicates nearly all leaves; used ONLY by
   movement.py:328 (`requires_open`).

They already disagree: `random_chance` always ÷100 in evaluation.py:216 vs
...
in the same elif chain (:454-494) — unreachable branches that will rot first.
...
- New conditions register once (ideally data-driven like effect_handlers —
