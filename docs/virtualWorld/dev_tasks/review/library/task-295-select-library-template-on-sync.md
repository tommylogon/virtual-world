...
- `POST /api/library/refresh-to-world` accepts an optional `template_id`; `_refresh_item` refreshes from that entry **and rebinds `node.properties['library_id']`** to it — a node can adopt a different library entry as its template (task-295).
...
- Natural sibling of `task-296` (mismatch warnings) — consider doing both in one "library sync" work session.
