...
**Source:** discussion — triggers can currently `spawn_item` but not ways.

## Goal

Add a `spawn_way` trigger effect so triggers (and behaviours) can materialize a way
node at runtime — e.g. a hidden passage opens when a condition is met, or an item's
`on_use` collapses/creates a corridor.

## Notes / open questions

- Ways are the only node type with **connection edges** (area → way with a `direction`,
  way → area). Spawning one is not a single `EDGE_IN` — it needs wire-up like
...
- Library exists at `data/library/ways/*.json` (name, description, current_state,
  see_through, auto_close, tags) — hydrate from it like `_hydrate_item` does for items.
...
- Undo/idempotency: spawning an already-existing way id — no-op, or reconnect?
- Options: generate the new area on the fly vs require an existing area id.

## Files

- `engine/effects.py` — add `handle_spawn_way` + `_hydrate_way` (mirror `_hydrate_item`).
- `engine/trigger_system.py` — `EFFECT_TYPES` + execution branch (or route through the
  shared effects path like `spawn_item`).
- `engine/trigger_validator.py` — schema/validation for the new params.
- `static/js/inspector/behaviors-view.js` + trigger editor — new effect entry + fields.
- `docs/virtualWorld/` trigger docs — list `spawn_way` as a supported effect type.