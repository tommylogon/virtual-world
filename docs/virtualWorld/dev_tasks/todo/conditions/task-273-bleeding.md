...
- **Periodic**: HP drain per tick per instance — multiple wounds sum (e.g. two wounds = −6 HP/tick). Severe hemorrhage via `level`.
...
- Drain balance: base `−3`/tick reasonably lethal but survivable with fast treatment.
