...
A wounded body part (light/moderate/severe). Uses `level` for severity and `ends_on: ["fix"]` so healing removes it. Core to injury → healing gameplay.
...
- **Movement**: a leg injury → `speed_mult`/`movement_mode` scaling; an arm injury → attack/use penalties.
...
- Is "severe injury" just a high `level`, or a separate condition (e.g. a broken bone → `prone` with `ends_on: ["fix"]`)?
