...
- **Movement**: `level_speed_mult` drops with severity (0.9 → 0.3).
- **Periodic**: escalating Energy + HP drain (level-scaled, like `exhausted`).
- **Onset & driver**: temperature system → once body temp crosses a threshold, apply `hypothermia`; `wet` accelerates it; insulation/warmth (fire, shelter, dry clothes) reverses it toward `ends_on: ["warm"]`.
...
- [ ] Unchecked severe hypothermia can collapse the character (HP 0 → unconscious/dead).
...
- Separate mild "chilled" → `hypothermia`, or all one condition with levels?
