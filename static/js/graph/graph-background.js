...
(() => {
    ...
    function reset() {
        ...
        if (state.zoneOverlay)
            state.zoneOverlay.style.pointerEvents = 'none';
        _render();
        _updateHint();
    }
    /* ── persistence ───────────────────────────────────────────────────── */
    ...
    function paintedGridRect(grid, scale) {
    ...
    function _startZoneDrag(event, scopeId) {
        ...
        const onUp = () => {
            ...
            if (typeof ApiClient !== 'undefined' && ApiClient.setScopeOffset) {
                ApiClient.setScopeOffset(scopeId, final).catch(() => { });
            }
            try {
    ...
    async function _layerForFit(payload, scopeId) {
     ...
     */
    function _applyReferenceLayout(layer, payload, gridRect, scopeId) {
     ...
     */
    async function fitToPaintedGrid() {
        const scopeId = _currentScopeId();
        if (!scopeId) {
        ...
        let payload = null;
        try {
            payload = await ApiClient.getWorldGrid(scopeId);
        }
        catch (error) {
            payload = null;
        }
    ...
    function _onKeyDown(event) {
        ...
        if (event.key === 'Escape' && state.zoneMove) {
            setZoneMove(false);
            return;
        }
        if (!state.editing)
    ...
    function _updateHint() {
        ...
        if (!state.editing && !state.zoneMove) {
            if (chip)
                chip.remove();
            return;
        }
    ...
    function showCanvasMenu(event) {
        ...
        rows.push(item('fit-grid', '▦ Fit to painted grid', false, '', _currentScopeId() ? '' : '#666'));
        // task-523: grab the whole zone (its painted nodes + art) and drag it.
        if (state.zoneMove || _canMoveZone()) {
            rows.push(item('zone-move', state.zoneMove ? '✔ Done moving zone' : '✥ Move zone', state.zoneMove));
        }
        rows.push(separator);
