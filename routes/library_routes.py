...
from .helpers import load_registry, save_registry, delete_registry_entry, _registry_subdir, validate_tags_on_save
...
# Map a library `contents` relation keyword to the corresponding graph edge type.
# Default relation for contained items is ``in`` (containment); spatial relations
# (on/under/behind/beside/at) attach the child to its parent as a surface/object.
RELATION_EDGE_TYPES = {
    "in": EDGE_IN,
    "on": EDGE_ON,
    "under": EDGE_UNDER,
...
def _library_type_count(data_dir, lib_type):
    """Count entries for a library type (directory may not exist yet)."""
    try:
...
def _lookup_library_item(app, item_id):
    """Fetch one item template from the items library registry (or None)."""
    items_reg = load_registry(app.config['DATA_DIR'], 'items.json')
    return items_reg.get(item_id)


def _content_ref_id(ref):
    """Coerce a contents entry to a library item id (string or {id:...} dict)."""
...
def _content_relation(ref, default="in"):
    """Return the relation keyword for a contents entry.

    Entries may carry an explicit ``relation`` (on/under/behind/beside/at/in).
    Falls back to ``default`` ('in') for plain string refs and older files.
    """
    if isinstance(ref, dict):
...
def graph_add_relation_edge(graph, source, target, relation="in"):
    """Add (or replace) a contained/spatial edge source -> target for a relation keyword."""
...
def graph_add_in_edge(graph, source, target):
    """Add (or replace) an ``in`` containment edge source -> target."""
    graph_add_relation_edge(graph, source, target, "in")


def _materialize_trigger_nodes(graph, node_id, trigger_data):
    """Clone one template trigger into a logic_trigger node + edge on the source node."""
...
def _spawn_library_item_node(app, item_id, lib_item, container_id=None):
    """Create a fresh unique instance node for a library item template.

    Materializes the item's contents (array of child *template ids*) recursively
    as unique child instances wired with ``in`` edges — the graph-based model.
    ...
    # Materialize contained items from the template's `contents` list. Entries
    # are library item-id refs (graph-aligned model). Legacy entries may still
    # embed full {id, name, ...} dicts — resolve by `id` when present. The
    # child's `relation` keyword decides the edge (default `in`): a spatial
    # relation (on/under/beside/behind/at) attaches it to this node as a
    # surface/position instead of simple containment.
    ...
    # Attach to the given container via `in` if this node is a child itself.
    if container_id:
        graph_add_in_edge(graph, node_id, container_id)

    # Materialize triggers from the template.
...
def _materialize_contained_items(app, parent_node_id, contents, logger_ctx=""):
    """Spawn each contained child (by library id) and link it to the parent node
    with its ``relation`` edge. Used when a top-level item is built directly
    (area import) so its spatial web is materialized like `_spawn_library_item_node`.
    """
...
def register_library_routes(app):
    """Register unified library CRUD and import routes."""

    # ── Entity Summary ────────────────────────────────────────────────

    @app.route('/api/library/entities', methods=['GET'])
    def library_entities():
        """Return a summary of all library entity types and their counts."""
    ...
    def library_list(registry_type):
        """List all entries of a registry type."""
    ...
    def library_all():
        """Return multiple registries in one round-trip (fast sync-list render).

        Optional ``?types=items,ways,areas`` narrows to the requested types;
        defaults to every REGISTRY_TYPES member.
        """
        data_dir = app.config['DATA_DIR']
    ...
    def library_create_or_update(registry_type):
        """Create or update an entry in a registry type."""
        if registry_type not in REGISTRY_TYPES:
            return jsonify({"error": f"Unknown registry type: {registry_type}"}), 400
    ...
    def library_delete(registry_type, entry_id):
        """Delete an entry from a registry type."""
        if registry_type not in REGISTRY_TYPES:
            return jsonify({"error": f"Unknown registry type: {registry_type}"}), 400
        filename = f"{registry_type}.json"
        registry = load_registry(app.config['DATA_DIR'], filename)
    ...
    def library_rename(registry_type, entry_id):
        """Rename a library entry: write the new file and remove the old one."""
        if registry_type not in REGISTRY_TYPES:
            return jsonify({"error": f"Unknown registry type: {registry_type}"}), 400
    ...
    def library_place_item(item_id):
        """Place a library item into the world (area, container, or character)."""
        data = request.get_json() or {}
        ...
        # Materialize contained children from the template's `contents` (library
        # item-id refs) as unique instances wired with `in` edges.
        for child_ref in (lib_item.get('contents') or []):
            child_id = _content_ref_id(child_ref)
            if not child_id:
                continue
    ...
    def library_import_character(char_id):
        """Import a character from the library into the world as an active player."""
        data = request.get_json() or {}
        ...
        # Place in specified area or character's original area
        ...
        # Import inventory items from library. Entries are either:
        #   * a library ID string (legacy) → look up in items library
        #   * a dict {name, library_id, node_id, properties} (canonical card)
        ...
        # Re-link equipped slots to the imported item nodes by name.
    ...
    def library_import_area(area_id):
        """Import a area from the library into the world graph."""
        data = request.get_json() or {}
    ...
    # ── Refresh from Library (unified) ────────────────────────────────

    @app.route('/api/ways/<node_id>/refresh-from-library', methods=['POST'])
    def refresh_way_from_library(node_id):
        """Apply selected sections from the way library entry to the graph node."""
        node = app.world.graph.get_node(node_id)
    ...
    def library_refresh_to_world():
        """Apply selected sections from the matching library entry to any node.

        Dispatches by node type (item / way). Prefers the node's ``library_id``
        property; ways fall back to the slug-of-name lookup for backward compat.
        """
        data = request.get_json() or {}
...
def _refresh_item(app, node, sections, template_id=None):
    """Reset an item node's properties and triggers from a library entry.

    ``sections`` = None → blind refresh of all unlocked props; a list → selective.
    ``template_id`` = optional override: refresh from that entry instead of the
    node's own ``library_id`` and rebind the node to it (task-295).
    """
...
def _refresh_way(app, node, sections, template_id=None):
    """Apply selected sections from the way library entry to a way node."""
...
def _refresh_area(app, node, sections, template_id=None):
    """Apply selected sections from the area library entry to an area node."""
    props = node.properties or {}
    area_id = template_id or props.get('library_id') or ''
    if not area_id:
        # Area library entries are keyed by slug id ("kitchen"), node ids are
        # "area_kitchen" — strip the prefix, then fall back to name slug.
        node_id = node.id or ''
...
def _refresh_character(app, node, sections, template_id=None):
    """Apply selected sections from a character library entry to a Player.

    Character library entries are keyed by name. The world's character data lives
    in the Player object (player_manager), not the (marker) graph node — so we
    update the Player's author-editable fields, skipping runtime state (vitals,
    current_area, memories, relationships, emotion, inventory).
    """
    props = node.properties or {}
    ...
    # Author-editable fields only — never overwrite runtime state.
    editable_map = {
        'name': 'name',
...
def _rebuild_triggers(app, node, lib_triggers):
    """Remove existing trigger edges/nodes for ``node`` and recreate them from ``lib_triggers``."""
    node_id = node.id
    old_trigger_ids = set()
