...
def register_players_routes(app):
    """Register player-related API routes (CRUD, movement, speech, knowledge, import)."""
    ...
    def api_spike_emotion(name):
        """Spike one affect dimension: {"emotion": "afraid", "delta": 12}.

        Used by the frontend for LLM-declared feelings and memory-recall
        re-spikes; the engine clamps and validates the dimension.
        """
        player = app.world.players.get(name)
        if not player:
            return jsonify({"error": "Player not found"}), 404
    ...
    def api_get_conditions():
        """Return the status-condition catalog for the inspector's condition editor."""
        catalog = []
    ...
    def api_player_speak(name):
        """Broadcast speech from a player or guest to the current area."""
        name = name.strip()
        data = request.get_json() or {}
        ...
        # Find the area - use specified area or current active player's area
        target_area = None
        ...
        # Broadcast speech to area (also records turn event internally via broadcast_speech)
    ...
    def api_update_player(name):
        ...
        if "new_name" in data and data["new_name"] and data["new_name"] != old_name:
            ...
            # Remove from players dict and re-insert with new name
            ...
            if node:
                node.name = new_name
                node.id = new_node_id  # Change ID
                # Update all edges that point to/from this node
                ...
                # Replace node in graph dict
                app.world.graph.nodes[new_node_id] = node
                del app.world.graph.nodes[old_node_id]

            # If this was the active player, update active_player name
        ...
        if "remove_condition" in data:
            ...
            if isinstance(cid, str) and cid in player.conditions:
                ...
                # Grapple sync: dropping grappled removes the grappled edge in
                # BOTH directions (someone holding this player, or this player
                # holding someone else) — edge is authoritative for the pairing.
                if cid == "grappled":
                    try:
    ...
    def import_player():
        ...
        # Emotion: export card has {current, intensity}
        ...
        # Current area — validate it exists
        ...
        for item_name in inventory:
            ...
            # Remove any existing carrying edge and add to player
    ...
    def api_generate_character_description(name):
        """Regenerate character appearance via LLM from base_description + equipment."""
        if name not in app.world.players:
            return jsonify({"error": "No such player"}), 404
        try:
            player = app.world.players[name]
    ...
    def api_get_vital_detail(name, vital_name):
        """Return detailed info about a specific vital: value, max, decay rate, time to empty, affecting conditions."""
        if name not in app.world.players:
            return jsonify({"error": "Player not found"}), 404
        player = app.world.players[name]
        ...
        if vital_name == "Temperature":
            ...
            # Drift calculation (mirrors tick_manager.py logic)
            ...
            # Time estimates (linear projection)
            ...
            if drift_direction == "cooling" and drift_rate > 0:
                time_est["to_hypothermia"] = max(0, round((core_temp - 33) / drift_rate, 1))
                time_est["to_death_cold"] = max(0, round((core_temp - 30) / drift_rate, 1))
                if core_temp <= 35:
                    time_est["to_comfortable"] = 0
                else:
                    time_est["to_comfortable"] = max(0, round((core_temp - 35) / drift_rate, 1))
            elif drift_direction == "warming" and drift_rate > 0:
                time_est["to_heat_stroke"] = max(0, round((40 - core_temp) / drift_rate, 1))
                time_est["to_death_heat"] = max(0, round((42 - core_temp) / drift_rate, 1))
                if core_temp >= 39:
                    time_est["to_comfortable"] = 0
                else:
                    time_est["to_comfortable"] = max(0, round((39 - core_temp) / drift_rate, 1))
            else:
                if core_temp < 35:
                    time_est["to_comfortable"] = round((35 - core_temp) / 0.1, 1)
                elif core_temp > 39:
                    time_est["to_comfortable"] = round((core_temp - 39) / 0.1, 1)
                else:
                    time_est["to_comfortable"] = 0
            ...
            if 35 <= core_temp < 37:
                dmg["energy"] += 1
            elif 33 <= core_temp < 35:
                dmg["energy"] += 2
                dmg["hp"] += 1
            elif core_temp < 33:
                dmg["hp"] += 3
            elif 37 < core_temp <= 38:
                dmg["thirst"] += 1
            elif 38 < core_temp <= 40:
                dmg["hp"] += 1
            elif core_temp > 40:
                dmg["hp"] += 3

            # Comfort status
            if 35 <= core_temp <= 39 and 15 <= eff_temp <= 30:
                comfort = "comfortable"
            elif core_temp < 33 or core_temp > 40:
                comfort = "dangerous"
            elif core_temp < 35 or core_temp > 38:
    ...
    def api_update_vital(name, vital_name):
        """Update a vital's value or decay rate."""
        if name not in app.world.players:
            return jsonify({"error": "Player not found"}), 404
        player = app.world.players[name]
        if vital_name not in player.vitals:
